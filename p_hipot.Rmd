...
#### Es la significancia estadística, representa el valor para la probabilidad de rechazar la hipótesis nula $H_0$, dado que $H_0$ es cierta y es conocido como el error tipo I. 
...
#### **Decisión**: Se decide rechazar la hipótesis nula $H_0$, dado a que $Z^* > Z_{0.05}$.
...
**El p-value es una probabilidad, y es la probabilidad de observar un valor igual o mayor al estadístico de prueba bajo el supuesto de que la hipótesis nula $H_0$ es cierta**. 
...
#### $$Z^*=\dfrac{\bar{X}-\mu_0}{\sigma/\sqrt{n}}=\dfrac{225.9 - 220}{13/\sqrt{22}}=\dfrac{5.9}{2.906888}=2.029$$
...
#### Para encontrar el "p-value", debemos calcular la siguiente probabilidad: $P(Z > Z^*)$, es decir $P(Z > 2.029)$. Para esto usamos RStudio.
...
#### Si la prueba es a cola izquierda la región de rechazo son los valores de $t < -t_{\alpha}$, y se decide rechazar $H_0$ si $t^* < -t_{\alpha}$.


#### Si la prueba es a cola derecha la región de rechazo son los valores de $t > t_{\alpha}$, y se decide rechazar $H_0$ si $t^* > t_{\alpha}$.


#### Si la prueba es a dos colas la región de rechazo son los valores de $t < -t_{\alpha}$ y los valores de $t > t_{\alpha}$, y se decide rechazar $H_0$ si $|t^*| > t_{\alpha}$.
...
#### 1. Las ventas diarias actuales de café molido regular en un supermercado son en promedio  de 55 libras, y varían según una distribución normal. La tienda quiere reducir el precio para vender intentar vender más café molido. Después de la reducción de precios, las ventas en las próximas tres semanas promediaron 63 libras, con una desviación estándar de 15. ¿Es esto una buena evidencia de que las ventas promedio son ahora de 360 libras? Construya un intervalo de confianza del 90 % para responder a la pregunta.
...
b) #### La varianza $\sigma$ es desconocida.

####  El estadístico de prueba bajo $H_0$ es

####  $$t^* =\dfrac{\bar{X}-\mu_0}{S/\sqrt{n}}=\dfrac{63-55}{18/\sqrt{43}}=2.914$$
...
#### **Decisión**: Se decide rechazar la hipótesis nula $H_0$, dado a que $t^* > t_{0.1,42}$.
...
#### $$Z^*=\dfrac{\hat{p}-p_0}{\sqrt{\dfrac{p_0(1-p_0)}{n}}}=\dfrac{0.54-0.5}{\sqrt{\dfrac{0.5(1-0.5)}{500}}}=1.789$$
...
<p class="comment">
La información de la salida  dice que `alternative hypothesis: true mean is greater than 55`. Entonces  $\mu_0$, es $55$. Por tanto las hipótesis respectivas fueron:
$$H_0:\mu=55$$
$$H_a:\mu > 55$$
</p>
...
#### Para este caso podemos usar la función `t.test()` disponible en la base de R. (No es necesario instalar ningún paquete). . La función `t.test()` tiene los siguientes argumentos:   `t.test(x, alternative = "two.sided", mu = 0, conf.level = 0.95)`
...
#### 1. Los grados de libertad **(df)** de la distribución t-Student fueron 14. Esto quiere decir que se usó una muestra de tamaño 15.


#### 2. El p-value es $0.5351$


#### 3. La información de la salida  dice que `alternative hypothesis: true mean is greater than 28`. Entonces  $\mu_0$, es $28$. Por tanto las hipótesis respectivas fueron:

$$H_0:\mu=28$$

$$H_a:\mu \neq 28$$

#### Aunque ya lo sabiamos!!


#### 4. La significancia es del 5 %, es decir $\alpha = 0.05$


#### 5. La media estimada es $\bar{X}=28.2666$
...
#### 1. Si no conociéramos los datos de entrada, la salida los proveé, la salida nos dice que `data:  29 out of 125`, es decir que $n=125$ y que $X=29$

#### 2. El p-value es $0.03333$.

#### 3. La información de la salida  dice que `alternative hypothesis: true p is less than 0.312`. Entonces  $p_0$, es $0.312$. Por tanto las hipótesis respectivas fueron:
...
#### 5. El valor de p, estimado a traves de  $\hat{p}$, es  $p = 0.208$
