...
Quiza estamos interesados en solo un subconjunto de una tabla de datos. Por ejemplo, en la tabla de datos `iris` solo queremos usar los datos de la especie `versicolor`, entonces para este trabajo podemos unsar la función de R `subset()`. Veamos un ejemplo.
...
Notemos que el primer argumento de la función `subset()` es el nombre de los datos o tabla de datos original, luego el segundo argumento es la condición que usted quiere que cumpla su nueva tabla. Que pasa si solo queremos usuar `Petal.Length` mayores a 5.8?
