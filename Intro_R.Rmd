...
Se pueden contruir mapas para mostrar la prevalencia por ejemplo de alguna enfermedad. En este caso tenemos un mapa elaborado en R que muestra los índices de terrorismo mundial. 
...
La función `paste()`  se usa para concatenar dos o mas objetos en R, veamos un ejemplo en conjunto con la función `print()`.
...
Un data frame en R se usa para almacenar tablas de datos. Para crear el data frame los vectores deben tener la misma longitud. Por ejemplo, vamos a contruir un data frame  con los vectores que construimos anteriormente: `GPA`, `Genero` y `Concentracion`, y le asignaremos el nombre `Students`. La función para crear un data frame es `data.frame`
...
### Leer datos interno en RStudio

RStudio tiene muchos datos que se utilizan para ilustrar los métodos y análisis estadísticos. Estos datos por lo general vienen incorporados en los paquetes, y la forma correcta de usarlos es con la función `data("dataset")`, dataset se refiere al nombre de los datos. Ejemplo, vamos a cargar los datos `iris`.
