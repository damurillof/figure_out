...
Si no hay “outliers” entonces las líneas laterales de la caja llegan hasta el valor mínimo por abajo, y hasta el valor máximo por arriba. Cuando hay “outliers” entonces éstos aparecen identificados en la figura y las lineas
laterales llegan hasta los valores adyacentes a las fronteras interiores. Si las lineas laterales son bastantes alargadas entonces significa que los extremos de la distribución de los datos se acercan lentamente al eje X.

Para dibujar un Boxplot en RStudio se puede usar la función `boxplot`. Veamos algunos ejemplos con los datos de la clase.

```{r,comment=NA}
# Leer los datos
Estudiantes <- read.table("/datos/DirectorioR/text.txt",header = T)
# Boxplot para estatura
boxplot(Estudiantes$estatura,data=Estudiantes,main="Estatura por estudiantes",xlab="", ylab="Estatura")
```


```{r,comment=NA}
# Boxplot para GPE
boxplot(Estudiantes$estatura,data=Estudiantes,main="GPA por estudiantes",xlab="Grupo sexo", ylab="GPA")
```

### Boxplot para grupos 

El boxplot es una herramienta interesante para visualizar graficamente la distribución de los datos por grupos, por ejemplo en los datos podemos visualizar la distribución de la estatura para mujeres y hombres por aparte. Veamos 

```{r,comment=NA}
# Boxplot para GPE
boxplot(Estudiantes$estatura~Estudiantes$sexo,data=Estudiantes,main="Estatura por estudiantes",xlab="", ylab="GPA")
```

**Observamos que la distribución de la estatura para los hombres es asimetrica a la izquierda**

Usando `ggplot2` para dibujar un boxplot para la variable `GPE` y el grupo sexo.
...
El resumen de cinco números es un conjunto de estadísticas descriptivas que proporcionan información sobre un conjunto de datos. Consiste en los cinco percentiles de muestra más importantes:
...
Un maestro quiere examinar los puntajes de los exámenes de los estudiantes. Sus puntajes son: 74, 88, 78, 90, 94, 90, 84, 90, 98 y 80.
