...
library(data.table)
Estudiantes <- fread("https://didier-murillo.netlify.com/estudiantes.txt",header = TRUE,sep = "\t")
head(Estudiantes)
```
...
Con los resultados de la tabla de frecuencia construida arriba para el pesos de los estudiantes y usando los puntos medios `MC` y las frecuencas absolutas  podemos hacer manualmente  un histograma de frecuencias para el `peso` de los estudiantes.
...
La función `hist()` tiene opciones para colocar el nombre o título del histograma, también para asignar nombres a los ejes x,y. También se puede cambiar el color del histograma. 
...
ggplot(data=asimetric1, aes(x = asimetric1)) + 
  geom_histogram(binwidth=0.2,col="black",fill="green") + ggtitle("Histograma Sesgado a la izquierda")  + xlab("GPA")
```
...
```{r, comment=NA,echo=F,eval=T,message=FALSE}
set.seed(123)
simetric <- rnorm(10000)
...
Se refiere a un histograma sesgado a la  derecha cuando las barras más altas estan al comienzo de la gráfica.
...
ggplot(data=asimetric2, aes(x = asimetric2)) + 
  geom_histogram(binwidth=0.2,col="black",fill="orange") + ggtitle("Histograma sesgado a la derecha")  + xlab("GPA") 
```
...
ggplot(Estudiantes, aes(x = Estudiantes$area,fill = Estudiantes$area)) + geom_bar(position="dodge") + 
  ggtitle("Grafico de barras para area del estudiante") + xlab("Area del estudiante") + ylab("Número de estudiantes") 
```
...
ggplot(Estudiantes, aes(x = Estudiantes$nota_esma,fill = Estudiantes$nota_esma)) + geom_bar(position="dodge") + 
  ggtitle("Grafico de barras para notas de los estudiante en ESMA") +
...
ggplot(Estudiantes, aes(fill=Estudiantes$trabaja,  x=Estudiantes$area)) + 
    geom_bar(position="dodge") + ggtitle("Grafico de barras por grupos para area y opción de trabajo.") +
  xlab("Area del estudiante") + ylab("Número de estudiantes") 
```
...
pie(num_Area,labels = Area, col=rainbow(length(Area)),
  	main="Gráfico circular para area")
```
