...
Estudiantes <- read.table("/datos/DirectorioR/figure_out/text",header = T)
head(Estudiantes)
```
...
O escrita manualmente con los resultados encontrados, como se ve a continuación:
...
Estudiantes1 <- read.table("/datos/DirectorioR/figure_out/text",header = T)
library(ggplot2)
ggplot(Estudiantes1, aes(x = Estudiantes1$edad)) + geom_dotplot(fill = "red", color = "black") + 
...
La siguiente función construye una tabla de frecuencias para variables continuas. Para usar esta función solo debe copiar el código  y ejecutarlo en su computador. 
frec_relativa, frec_acumu, frecuencia_abs_acum
...
Con los resultados de la tabla de frecuencia y usando los puntos medios y las frecuencas absolutas  podemos hacer un histograma para el `peso` de los estudiantes.
...
### Histogramas simétricos y asimétricos.


> Histograma Asimétrico a la Izquierda

Se refiere a un histograma asimétrico a la izquierda cuando hay menos datos al lado izquierdo del histograma.

```{r, comment=NA,echo=F,eval=T}

asimetric1 <- c(runif(25,2,3.3),runif(80,3.2,4))
asimetric1 <- as.data.frame(asimetric1)
ggplot(data=asimetric1, aes(x = asimetric1)) + 
  geom_histogram(binwidth=0.2,col="black",fill="green") + ggtitle("Histograma Asimétrico a la izquierda")  + xlab("GPA")
```
...
```{r, comment=NA,echo=F,eval=T}
simetric <- rnorm(1000,3,0.3)
simetric <- as.data.frame(simetric)
ggplot(data=simetric, aes(x = simetric)) + 
  geom_histogram(binwidth=0.2,col="red",fill="black") + ggtitle("Histograma Simétrico")  + xlab("GPA")
...
Se refiere a un histograma asimétrico a la derecha cuando hay menos datos al lado derecho del histograma.

```{r, comment=NA,echo=F,eval=T}
asimetric2 <- c(runif(80,2,2.6),runif(25,2.5,4))
asimetric2 <- as.data.frame(asimetric2)
ggplot(data=asimetric2, aes(x = asimetric2)) + 
  geom_histogram(binwidth=0.2,col="black",fill="orange") + ggtitle("Histograma Asimétrico a la derecha")  + xlab("GPA") 
```
...
Las frecuencias acumuladas no son importantes si la variables es nominal como el caso de la variable `area`
